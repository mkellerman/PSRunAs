function Invoke-ExpressionAs {
    ...
    Begin {
        ...
        #>
            ...
            if (-not $Result) { throw 'Unable to create process as user.'; Return }

            Write-Verbose 'Process created successfully!'
            Get-Process -Id $ProcInfo.dwProcessId
    ...
    Process {
        ...
        $CmdCredential = @{}
        If ($Credential) {
            $CmdCredential['Username'] = $($Credential).GetNetworkCredential().Username
        ...
        $CmdCommand = "cmd /c powershell.exe -NoLogo -ExecutionPolicy Bypass -NonInteractive -NoProfile -OutputFormat Text -EncodedCommand $EncodedCommand >$fStdOutput"
        $Process = Start-ProcessAsUser @CmdCredential -Cmd $CmdCommand -NetOnly
        $Process.WaitForExit()
