...
"@
	        ...
	        Write-Debug "`n[>] Calling Advapi32::CreateProcessWithLogonW"
	        ...
	        if (!$CallResult) {
		        Write-Warning "`n[!] Mmm, something went wrong! GetLastError returned:"
		        Write-Warning "==> $((New-Object System.ComponentModel.Win32Exception([int][Kernel32]::GetLastError())).Message)`n"
	        } else {
		        Write-Debug "`n[+] Success, process details:"
		        Get-Process -Id $ProcessInfo.dwProcessId
	        }
        }
    
        $fStdOutput = [System.IO.Path]::GetTempFileName()
        $fStdObject = [System.IO.Path]::GetTempFileName()
    ...
    Process {
        ...
        $CmdCredential = @{}
        If ($Credential) {
            $CmdCredential['User'] = $($Credential).GetNetworkCredential().Username
            $CmdCredential['Password'] = $($Credential).GetNetworkCredential().Password
            $CmdCredential['Domain'] = $($Credential).GetNetworkCredential().Domain
            If (!$CmdCredential['Domain']) { $CmdCredential['Domain'] = '.' }
        }
        $Command = "`$(`r`n$Command`r`n) | Export-Clixml $fStdObject *>&1"
        $EncodedCommand = [Convert]::ToBase64String(([System.Text.Encoding]::Unicode.GetBytes(([ScriptBlock]::Create($Command)))))
        $CmdCommand = "cmd.exe"
        $CmdArguments = "/c powershell.exe -NoLogo -ExecutionPolicy Bypass -NonInteractive -NoProfile -OutputFormat Text -EncodedCommand $EncodedCommand >$fStdOutput"
        $Process = Invoke-RunAs @CmdCredential -Binary $CmdCommand -Args $CmdArguments -LogonType 2
        $Process.WaitForExit()
        Get-Content -Path $fStdOutput -EA 0 | % { Write-Host $_ -ForegroundColor Gray }
        [object[]]$RetObject = Import-Clixml $fStdObject -EA 0
        Return $RetObject
    ...
    End {
        ...
        Remove-Item -Path $fStdOutput -EA 0
        Remove-Item -Path $fStdObject -EA 0
